class Help_messages():
    def __init__(self):
        ...
        self.r = "使用方法：\n.r[dah#bp] a_number [+/-]ex_number\n" \
            ...
            "a：检定指令，根据后续a_number设定数值检定；非1D100判定将被忽略。\n" \
            "h：暗骰指令，骰子结构将会私聊发送给该指令者；（频道暂不支持）\n" \
            "#：多轮投掷指令，#后接数字即可设定多轮投掷；\n" \
            "bp：奖励骰与惩罚骰；\n" \
            "+/-：附加计算指令，目前仅支持数字\n" \
            "例如 .r#2bba 70：两次投掷 1D100 ，附加两个奖励骰，判定值为70"
        self.sc = "使用方法：\n.sc success/failure san_number\n" \
            "success：判定成功降低的san值，支持x或xdy语法（x与y为数字）；\n" \
            "failure：判定失败降低的san值，支持语法如上；\n" \
            "san_number：当前san值，缺省san_number将会自动使用保存的人物卡数据。"
        self.coc = "使用方法：\n.coc age\n" \
            "age：调查员年龄\n" \
            "调用完后调查员资料存入暂存区，需要输入 .st 来将暂存区的调查员资料保存下来"
        self.st = "使用方法：\n.st [name]-[attr_name1][attr_num1][attr_name2][attr_num2]...\n" \
            "name：调查员名字，可以没有，但要去掉-\n" \
            "attr_name：属性/技能名称，例 str、力量、会计\n" \
            "attr_num：属性值\n" \
            "可以使用自动人物卡中的快捷输入指令\n" \
            "例如 .st 王嘉然-\n" \
            "例如 .st 王嘉然-力量40敏捷80\n" \
            "例如 .st 力量40敏捷80估价5\n" \
            "可以单独输入.st指令，骰娘将自动读取最近一次coc指令结果进行保存"
        self.show = "使用方法：\n.show [s] [@xxx]\n" \
