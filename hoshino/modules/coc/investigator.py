...
class Investigator(object):
    ...
    def __repr__(self) -> str:
        return "%s 年龄:%d\n力量:%d 体质:%d 体型:%d\n敏捷:%d 外貌:%d 智力:%d\n意志:%d 教育:%d 幸运:%d\nDB:%s 生命值:%d/%d 魔法:%d/%d 移动速度:%d 理智:%d" % (
            self.name, self.age, self.str, self.con, self.siz, self.dex, self.app, self.int, self.pow, self.edu, self.luc, self.db(), self.lp, self.lp_max(), self.mp, self.mp_max(), self.mov(), self.san)
